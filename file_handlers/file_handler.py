...
class GeorgiaFileHandler:
    ...
    def get_file(self) -> str:
        ...
        ftp = ftplib.FTP()
        try:
            ftp.connect(settings.georgia_settings.ftp_server, settings.georgia_settings.ftp_port, timeout=20)
            ftp.login(settings.georgia_settings.ftp_user, settings.georgia_settings.ftp_password)
        except TimeoutError as err:
            logger.error(f'could not connect to {settings.georgia_settings.ftp_server}:{settings.georgia_settings.ftp_port}')
            raise GetFileError()
